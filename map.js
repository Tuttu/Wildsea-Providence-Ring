...
'overflowImages': [
	...
	{
		'src': 'Trail.svg',
		'hex': 'SqwikerTrail, RotipedeTunnels',
		'width': 26,
		'height': 22
	},
	{
...
ready: function() {
    ...
    hexmap.on('mouseover',function(e){
        let tip = hexmap.el.querySelector('.tooltip');
        if(!tip){
            tip = document.createElement('div');
            tip.classList.add('tooltip');
            hexmap.el.appendChild(tip);
        }
        tip.style.display = 'block';
        tip.innerHTML = e.data.data.n+' ('+e.data.data.q+','+e.data.data.r+')<br />Region: '+(e.data.data.region || 'N/A');
        const bb = e.target.getBoundingClientRect();
        const bbo = hexmap.el.getBoundingClientRect();
        tip.style.left = Math.round(bb.left + bb.width/2 - bbo.left + hexmap.el.scrollLeft)+'px';
    ...
    filterInput.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            // Hide the entire section (header and content) if no items are visible
    ...
    // --- 4. Legend Hover Interactivity (POI and Region) ---
    ...
    regionLegendItems.forEach(item => {
        ...
        if (regionId && boundaryPath) {
            item.addEventListener('mouseover', () => boundaryPath.classList.add('boundary-highlight'));
            item.addEventListener('mouseout', () => boundaryPath.classList.remove('boundary-highlight'));
        }
    });
  }
});
